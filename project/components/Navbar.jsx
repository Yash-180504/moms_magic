...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    if (selected?.userId === user.id && selected.address) {
      ...
      localStorage.setItem(
        "mm_selected_address",
        JSON.stringify({ userId: user.id, address: userAddresses[0] }),
      );
    }
  }, [user]);
  ...
  function persistAddressBook(newAddresses) {
    ...
    const selected =
      newAddresses.find((a) => a.id === selectedAddress?.id) ||
      newAddresses[0] ||
      null;
    if (selected) {
      ...
      localStorage.setItem(
        "mm_selected_address",
        JSON.stringify({ userId: user.id, address: selected }),
      );
    }
  }
  ...
  function removeAddress(id) {
    ...
    if (selectedAddress?.id === id) {
      ...
      localStorage.setItem(
        "mm_selected_address",
        JSON.stringify({ userId: user.id, address: next }),
      );
    }
  }
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center gap-3 h-14 md:h-16">
          ...
          >
            ...
            <span className="font-heading font-bold text-lg md:text-xl text-[#0F172A]">
              Mom&apos;s Magic
            </span>
          </Link>
          ...
          >
            ...
            {(!user || user.role === "customer" || user.role === "admin") && (
              <Link
                href="/cart"
                className="relative flex items-center gap-2 text-sm font-semibold text-[#0F172A] bg-[#FDF4F0] border border-[#FCEAE1] rounded-xl px-3 py-2 hover:border-[#EA580C] transition-all"
                aria-label="Cart"
              >
                <ShoppingCart
                  size={16}
                  ...
                  {userMenuOpen && (
                    <>
                      ...
                      <div className="absolute right-0 mt-2 w-52 bg-white rounded-2xl border border-[#FCEAE1] shadow-xl z-20 overflow-hidden">
                        ...
                        >
                          <User size={15} className="text-[#64748B]" /> My
                          Profile
                        </Link>
                        <Link
                          href="/orders"
