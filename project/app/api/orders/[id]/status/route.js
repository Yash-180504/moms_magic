...
export async function PUT(request, { params }) {
  ...
  const roleErr = checkRole(payload, "customer", "provider", "admin");
  ...
  try {
    ...
    if (payload.role === "customer") {
      ...
      if (status !== "cancelled")
        return err("Customers can only cancel orders", 403);
      if (order.status !== "pending")
        return err("Only pending orders can be cancelled", 400);
