...
function Field({ label, id, children }) {
  return (
    <div>
      <label htmlFor={id} className="block text-xs font-semibold text-[#64748B] uppercase tracking-wide mb-1.5">{label}</label>
      {children}
    </div>
  )
}
      ...
      <div className="min-h-screen flex flex-col bg-[#FFF7ED]">
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-4 px-4 py-16">
          ...
          <div className="flex gap-3">
            <Link href="/login" className="bg-[#EA580C] text-white font-semibold px-6 py-3 rounded-xl hover:bg-[#C2410C] transition-colors text-sm">Log in</Link>
            <Link href="/register" className="border-2 border-[#EA580C] text-[#EA580C] font-semibold px-6 py-3 rounded-xl hover:bg-[#FFF7ED] transition-colors text-sm">Sign up</Link>
          </div>
        </div>
        <Footer />
      </div>
      ...
      <main className="flex-1 max-w-lg mx-auto w-full px-4 py-8 pb-6">
        {flash && (
          <div className={`flex items-center gap-2 rounded-xl px-4 py-3 mb-5 text-sm ${
            ...
            {flash.type === 'success' ? <CheckCircle size={16} className="shrink-0" /> : <AlertCircle size={16} className="shrink-0" />}
            {flash.msg}
          </div>
        )}
        ...
        <div className="bg-white rounded-2xl border border-[#FCEAE1] p-6 mb-4 flex items-center gap-4">
          <div className="w-16 h-16 bg-[#EA580C] rounded-2xl flex items-center justify-center text-white font-heading font-bold text-2xl shrink-0">{initials}</div>
          <div className="flex-1 min-w-0">
            ...
            <span className="inline-block mt-1 text-xs font-semibold text-[#EA580C] bg-[#FFF7ED] border border-[#FCEAE1] px-2.5 py-0.5 rounded-full capitalize">{user.role}</span>
          </div>
          {!editing && (
            <button onClick={() => setEditing(true)} className="p-2 text-[#64748B] hover:text-[#EA580C] hover:bg-[#FDF4F0] rounded-xl transition-colors cursor-pointer" aria-label="Edit profile">
              <Pencil size={18} />
            </button>
          )}
        </div>
          ...
          <div className="bg-white rounded-2xl border border-[#FCEAE1] p-5 mb-4">
            ...
            <form onSubmit={saveProfile} className="space-y-4">
              ...
              <Field label="Phone" id="phone">
                <input id="phone" type="tel" value={form.phone} onChange={e => setForm(p => ({ ...p, phone: e.target.value }))} placeholder="Optional"
                  className="w-full bg-[#FFF7ED] border border-[#FCEAE1] rounded-xl px-3 py-2.5 text-sm text-[#0F172A] outline-none focus:border-[#EA580C] focus:ring-2 focus:ring-[#EA580C]/20 transition-all" />
              </Field>
                ...
                <div key={key}>
                  ...
                  <div className="relative">
                    ...
                    <button type="button" onClick={() => setShowPw(p => ({ ...p, [key]: !p[key] }))}
                      className="absolute right-3 top-1/2 -translate-y-1/2 text-[#64748B] cursor-pointer" aria-label={showPw[key] ? 'Hide' : 'Show'}>
        ...
        <div className="bg-white rounded-2xl border border-[#FCEAE1] divide-y divide-[#FCEAE1] mb-4 overflow-hidden">
          {user.role === 'customer' && (
            <Link href="/orders" className="flex items-center gap-3 px-5 py-4 hover:bg-[#FDF4F0] transition-colors">
          ...
          {(user.role === 'provider' || user.role === 'admin') && (
            <Link href="/dashboard" className="flex items-center gap-3 px-5 py-4 hover:bg-[#FDF4F0] transition-colors">
